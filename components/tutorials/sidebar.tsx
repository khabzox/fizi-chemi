...
interface TutorialData {
  [categoryKey: string]: Record<string, Semester>;
}
...
type OpenSemesterKeys = Record<string, boolean>;
type OpenSubjectKeys = Record<string, boolean>;

type CategoryRefs = Record<string, HTMLDivElement | null>;
type SemesterRefs = Record<string, HTMLDivElement | null>;
type SubjectRefs = Record<string, HTMLUListElement | null>;

function Sidebar({ tutorialData, className }: SidebarProps) {
  const [data, setData] = useState<TutorialData>(tutorialData);
  const [openCategoryKeys, setOpenCategoryKeys] = useState<string[]>([]);
  const [openSemesterKeys, setOpenSemesterKeys] = useState<OpenSemesterKeys>(
    {}
  );
  const [openSubjectKeys, setOpenSubjectKeys] = useState<OpenSubjectKeys>({});

  const categoryRefs = useRef<CategoryRefs>({});
  const semesterRefs = useRef<SemesterRefs>({});
  const subjectRefs = useRef<SubjectRefs>({});
  ...
  const handleSemesterClick = (categoryKey: string, semesterKey: string) => {
  ...
  function choseLevelTitle(categoryKey: string): string {
    switch (categoryKey) {
      case "1ac":
        return "1ère Année Collège";
      case "2ac":
        return "2ère Année Collège";
      case "3ac":
        return "3ère Année Collège";
      case "tc":
        return "Tronc Commun";
      case "1bac":
        return "1ère Bac";
  ...
  return (
    <aside
      className={cn(
        `bg-white rounded-lg border-2 border-primary p-6`,
        className
      )}
    >
              ...
              <div key={semesterKey} className="mb-2">
                ...
                >
                  ...
                  <ChevronDown
                    className={cn("transition-transform duration-300", {
                      "rotate-180":
                        openSemesterKeys[`${categoryKey}-${semesterKey}`],
                    })}
                  />
                </div>
                <div
                  ref={(el) => {
                    semesterRefs.current[`${categoryKey}-${semesterKey}`] = el;
                  }}
                            ...
                            >
                              {Object.entries(subject.sections).map(
                                ([sectionKey, section]) => (
                                  <li
                                    key={sectionKey}
                                    className="text-sm text-primary bg-muted-hover p-1 rounded-md border-l-2 border-primary pl-4 hover:underline"
                                  >
                                    <Link
                                      href={`/tutorials/${categoryKey}#tutorials#semester_1#chemistry#${sectionKey}`}
