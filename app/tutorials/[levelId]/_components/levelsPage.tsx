...
const LevelsPage = ({ tutorialData, levelId }: LevelsPageProps) => {
  const { user } = useUser();
  ...
  interface FileLocationItem {
    key: string;
    title: string;
  }
  
  interface FormDataLesson {
    title: string;
    downloadLink: string;
    fileLocation: FileLocationItem[];
  }
  ...
  type AccordionType = "semester" | "subject" | "section";
  ...
  const containerTutorialsRef = useRef<HTMLDivElement | null>(null);
  ...
  const handleHash = (parts: string[]): void => {
    if (parts.length > 0) {
      const [semesterKey, subjectKey, sectionKey] = parts;
      setOpenItems({
        semester: semesterKey ? `semester-${semesterKey}` : null,
        subject: subjectKey ? `subject-${subjectKey}` : null,
        section: sectionKey ? `section-${sectionKey}` : null,
      });
    }
  };
  ...
  useEffect(() => {
    const targetId: string | null = openItems.section;
    if (targetId) {
  ...
  const handleAccordionTrigger = (key: string, type: AccordionType): void => {
    setOpenItems((prev: OpenItemsState) => ({
      ...prev,
      [type]: prev[type] === key ? null : key,
    }));
  };
...
const handleEdit = (id: string, currentTitle: string): void => {
  ...
  const handleSave = async ({
    ...
    try {
      ...
      if (sectionSnap.exists()) {
        ...
        const existingDataTyped: ExistingData = existingData as ExistingData;

        const updatedFiles: File[] = existingDataTyped.files.map((file: File) =>
        ...
        type SemestersState = Record<string, SemesterData>;

        setSemesters((prevSemesters: SemestersState) => {
            const updatedSemesters: SemestersState = { ...prevSemesters };
            const semesterData: SemesterData = updatedSemesters[semester];
            const subjectData: SubjectData = semesterData.subjects[subject];
            const sectionData: SectionData = subjectData.sections[section];

            sectionData.files = sectionData.files.map((file: FileData) =>
                file.id === id ? { ...file, title: newTitle } : file
            );
  ...
  const handleDelete = async ({
    ...
    interface ExtractFilePathFromUrl {
      (url: string): string;
    }

    const extractFilePathFromUrl: ExtractFilePathFromUrl = (url: string): string => {
    ...
    try {
      ...
      if (sectionSnap.exists()) {
        ...
        const updatedFiles: FileData[] = existingDataTyped.files.filter(
          (file: FileData) => file.id !== id
        );
        ...
        setSemesters((prevSemesters: SemestersState) => {
            ...
            sectionData.files = sectionData.files.filter(
                (file: FileData) => file.id !== id
            );
            return updatedSemesters;
        });
  ...
  const uniqueSectionTitles = useMemo(() => {
    if (selectedSemester && selectedSubjects) {
      const formattedSemesterName = getFormattedSemesterName(selectedSemester);
      const formattedSectionName = getFormattedSectionsName(selectedSection);
      const semesterData = semesters[formattedSemesterName];
      const subjectData = semesterData?.subjects?.[selectedSubjects];
      return subjectData?.sections ? Object.keys(subjectData.sections) : [];
    }
    return [];
  }, [selectedSemester, selectedSubjects, selectedSection, semesters]);
  ...
  useEffect(() => {
    ...
    setFormDataLesson((prev) => ({
      ...
      fileLocation: [
        { key: levelId, title: "" },
        { key: semester, title: selectedSemester },
        { key: subject, title: getFormattedSubjectsName(selectedSubjects) },
        { key: section, title: selectedSection },
      ],
    }));
  }, [levelId, selectedSemester, selectedSubjects, selectedSection]);

interface UploadFileSnapshot {
...
interface SendFormDataLessonEvent extends React.FormEvent<HTMLFormElement> {}

interface SendFormDataLessonResponse {
    ok: boolean;
    status: number;
}

async function sendFormDataLesson(e: SendFormDataLessonEvent): Promise<void> {
    e.preventDefault();

    setUploading(true);

    try {
        const res: SendFormDataLessonResponse = await fetch("/api/tutorial", {
            method: "POST",
            headers: {
...
const handleDragEnd = async (
    ...
    const reorderedFiles = Array.from(sectionData.files) as FileData[];
    const [removed] = reorderedFiles.splice(result.source.index, 1);
    reorderedFiles.splice(result.destination.index, 0, removed);

    setSemesters((prevSemesters: SemestersState) => {
        const updatedSemesters: SemestersState = { ...prevSemesters };
        updatedSemesters[semesterKey].subjects[subjectKey].sections[
            sectionKey
    ...
    try {
        setReorderSaving((prev: Record<string, boolean>) => ({ ...prev, [sectionKey]: true }));
        ...
        setReorderSaving((prev: Record<string, boolean>) => ({ ...prev, [sectionKey]: false }));
    } catch (error) {
...
interface ChoseLevelTitleMap {
    [key: string]: string;
}

function choseLevelTitle(levelId: string): string {
    const levelTitles: ChoseLevelTitleMap = {
        "1ac": "1ère Année Collège",
        "2ac": "2ère Année Collège",
        "3ac": "3ère Année Collège",
        "tc": "Tronc Commun",
        "1bac": "1ère Bac",
        "2bac": "2ème Bac",
    };
    return levelTitles[levelId] ? levelTitles[levelId] : "No Title";
}

  return (
    <TutorialsLayout
      tutorialData={tutorialData}
      title={choseLevelTitle(levelId)}
      pathName={levelId}
      path={`tutorials/${levelId}`}
    >
        ...
        {isAdmin && (
          <div className="p-4 bg-gray-100 border-b border-primary rounded-t-lg">
            <div className="pb-4">
              ...
              {alertFileIsUploaded && (
                <Alert variant="info">{alertFileIsUploaded}</Alert>
              )}
            </div>
            <div className="mb-4">
                  ...
                  {Object.entries(semester.subjects).map(
                    ([subjectKey, subject]) => (
                      <Accordion
                        ...
                        key={subjectKey}
                        value={
                          openItems.subject === `subject-${subjectKey}`
                            ? `subject-${subjectKey}`
                            : undefined
                        }
                      >
                              ...
                              ([sectionKey, section]) => (
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    >
                                      {section.title}
                                      {reorderSaving[sectionKey] && (
                                        <span className="ml-2 text-xs text-primary">saving...</span>
                                      )}
                                    </AccordionTrigger>
                                    <AccordionContent>
                                      {Object.keys(section.files).length ===
                                      0 ? (
                                                      ...
                                                      {(provided, snapshot) => (
                                                        ...
                                                        >
                                                          {isAdmin && (
                                                            ...
                                                            >
                                                              <span className="drag-icon cursor-move">
                                                                <Grip
                                                                  size={12}
                                                                />
                                                              </span>
